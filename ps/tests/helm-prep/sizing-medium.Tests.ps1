...
Describe 'Specifying Medium system size...' -Tag 'size' {
	...
	It 'Core feature should not include reservations' {
		...
		# a default config.json with Medium system size
		@'
{
  "configVersion": "1.3",
  "namespace": "srm",
...
'@.Replace('~/.k8s-srm', $workDirPath) | Out-File $configJsonPath
		...
		$yaml.GetKeyValue(('mariadb','master','resources','limits','cpu')) | Should -Be '8000m'
		$yaml.GetKeyValue(('mariadb','master','resources','limits','memory')) | Should -Be '32768Mi'

		$yaml.GetKeyValue(('mariadb','slave','resources','limits','cpu')) | Should -Be '4000m'
		$yaml.GetKeyValue(('mariadb','slave','resources','limits','memory')) | Should -Be '16384Mi'

		$yaml.GetKeyValue(('web','persistence','size')) | Should -BeNullOrEmpty

		$yaml.GetKeyValue(('mariadb','master','persistence','size')) | Should -Be '384Gi'

		$yaml.GetKeyValue(('mariadb','slave','persistence','size')) | Should -Be '384Gi'
		$yaml.GetKeyValue(('mariadb','slave','persistence','backup','size')) | Should -Be '1152Gi'
	}
    ...
    $testDrivePath = Get-TestDriveDirectoryInfo
		...
		# a default config.json with Medium system size and Tool Orchestration
		@'
{
  "configVersion": "1.3",
  "namespace": "srm",
...
'@.Replace('~/.k8s-srm', $workDirPath) | Out-File $configJsonPath
		...
		$yaml.GetKeyValue(('minio','persistence','size')) | Should -Be '128Gi'
	}
}