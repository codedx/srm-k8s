...
Describe 'Specifying Large system size...' -Tag 'size' {
	...
	It 'Core feature should not include reservations' {
		...
		# a default config.json with Large system size
		@'
{
  "configVersion": "1.3",
  "namespace": "srm",
...
'@.Replace('~/.k8s-srm', $workDirPath) | Out-File $configJsonPath
		...
		$yaml.GetKeyValue(('sizing','size')) | Should -Be 'Large'
		...
		$yaml.GetKeyValue(('mariadb','master','resources','limits','cpu')) | Should -Be '16000m'
		$yaml.GetKeyValue(('mariadb','master','resources','limits','memory')) | Should -Be '65536Mi'

		$yaml.GetKeyValue(('mariadb','slave','resources','limits','cpu')) | Should -Be '8000m'
		$yaml.GetKeyValue(('mariadb','slave','resources','limits','memory')) | Should -Be '32768Mi'

		$yaml.GetKeyValue(('web','persistence','size')) | Should -BeNullOrEmpty

		$yaml.GetKeyValue(('mariadb','master','persistence','size')) | Should -Be '768Gi'

		$yaml.GetKeyValue(('mariadb','slave','persistence','size')) | Should -Be '768Gi'
		$yaml.GetKeyValue(('mariadb','slave','persistence','backup','size')) | Should -Be '2304Gi'
	}
    ...
    $testDrivePath = Get-TestDriveDirectoryInfo
		...
		# a default config.json with Large system size and Tool Orchestration
		@'
{
  "configVersion": "1.3",
  "namespace": "srm",
...
'@.Replace('~/.k8s-srm', $workDirPath) | Out-File $configJsonPath
		...
		$yaml.GetKeyValue(('minio','persistence','size')) | Should -Be '256Gi'
	}
}