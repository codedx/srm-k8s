...
Describe 'Specifying ExtraSmall system size...' -Tag 'size' {
	...
	It 'Core feature should not include reservations' {
		...
		# a default config.json with ExtraSmall system size
		@'
{
  "configVersion": "1.3",
  "namespace": "srm",
...
'@.Replace('~/.k8s-srm', $workDirPath) | Out-File $configJsonPath
		...
		$yaml.GetKeyValue(('sizing','size')) | Should -Be 'ExtraSmall'
		...
		$yaml.GetKeyValue(('mariadb','slave','resources','limits','memory')) | Should -Be '4096Mi'

		$yaml.GetKeyValue(('web','persistence','size')) | Should -BeNullOrEmpty

		$yaml.GetKeyValue(('mariadb','master','persistence','size')) | Should -Be '96Gi'
		
		$yaml.GetKeyValue(('mariadb','slave','persistence','size')) | Should -Be '96Gi'
		$yaml.GetKeyValue(('mariadb','slave','persistence','backup','size')) | Should -Be '288Gi'
	}
    ...
    $testDrivePath = Get-TestDriveDirectoryInfo
		...
		# a default config.json with ExtraSmall system size and Tool Orchestration
		@'
{
  "configVersion": "1.3",
  "namespace": "srm",
...
'@.Replace('~/.k8s-srm', $workDirPath) | Out-File $configJsonPath
		...
		$yaml.GetKeyValue(('minio','resources','limits','cpu')) | Should -Be '1000m'
		$yaml.GetKeyValue(('minio','resources','limits','memory')) | Should -Be '2560Mi'

		$yaml.GetKeyValue(('argo-workflows','controller','resources','limits','cpu')) | Should -Be '250m'
		$yaml.GetKeyValue(('argo-workflows','controller','resources','limits','memory')) | Should -Be '250Mi'
		...
		$yaml.GetKeyValue(('minio','persistence','size')) | Should -Be '32Gi'
	}
}