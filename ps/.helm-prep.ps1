...
try {
	...
	if ($generatedValuesFiles.Length -gt 0) {
		...
		if ($skipYamlMerge) {
			...
			try {
				...
				$mergedValuesPath = Get-CombinedValuesPath $config
	...
	# Add values file(s) that will be generated by "Required K8s Scripts"
	if ($config.ingressType -eq [IngressType]::Route -and -not $config.skipScanFarm) {
		$allValuesFiles += (Get-OpenShiftRouteScanFarmTlsValuesPath $config)
	}
